...
- Verify the MSI or ZIP against `SHA256SUMS.txt`.
- If GitHub marks this as a prerelease and the build is unsigned, Windows may show a reputation warning.

Both downloads are self-contained and need no separate .NET installation. The MSI stores user data under `%LOCALAPPDATA%`; the portable ZIP keeps it beside the executable.
