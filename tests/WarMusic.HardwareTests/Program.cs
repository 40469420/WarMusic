...
if (args.Contains("--live-route"))
{
    ...
    var sb = new BufferedWaveProvider(ProcessCapture.Format, TimeSpan.FromSeconds(10)); var ab = new BufferedWaveProvider(ProcessCapture.Format, TimeSpan.FromSeconds(10)); var cb = new BufferedWaveProvider(cable.WaveFormat, TimeSpan.FromSeconds(10));
...
if (args.Contains("--spotify-route"))
{
    ...
    using var receiving = new WasapiCapture(device); var buffer = new BufferedWaveProvider(receiving.WaveFormat, TimeSpan.FromSeconds(5)) { DiscardOnBufferOverflow = true }; receiving.DataAvailable += (_, a) => buffer.AddSamples(a.Buffer, 0, a.BytesRecorded); receiving.StartRecording();
...
Check("Peak limiter bounds summed sources", () => { var m = new MixProcessor(); var send = new float[960]; var monitor = new float[960]; m.Process(Filled(2), Filled(2), Filled(2), send, monitor, 960, o, true, false, false, true); Assert(send.All(x => x <= .96f), "Limiter exceeded ceiling"); Assert(m.Limited, "Limiter indicator missing"); });
...
if (args.Contains("--engine-test"))
{
    try
    {
        ...
        var buffer = new BufferedWaveProvider(loopback.WaveFormat, TimeSpan.FromSeconds(3)) { DiscardOnBufferOverflow = true }; loopback.DataAvailable += (_, a) => buffer.AddSamples(a.Buffer, 0, a.BytesRecorded); loopback.StartRecording();
...
if (args.Contains("--routing-regression"))
{
    try
    {
        ...
        var gameBuffer = new BufferedWaveProvider(gameInput.WaveFormat, TimeSpan.FromSeconds(2)) { DiscardOnBufferOverflow = true }; gameInput.DataAvailable += (_, a) => gameBuffer.AddSamples(a.Buffer, 0, a.BytesRecorded); gameInput.StartRecording();
    ...
    var thread = new Thread(() =>
    {
        ...
        dispatcher.BeginInvoke(async () =>
        {
            try
            {
                ...
                if (!source.HasExited) { source.Kill(); await source.WaitForExitAsync(); }
                using var replacement = Process.Start(start)!; await Task.Delay(1600); await vm.Recover(); Assert(vm.Engine.SourcePid == replacement.Id && !vm.Engine.Transmitting, "Restarted source was not recovered muted");
                vm.DisconnectCommand.Execute(null); await vm.Recover(); Assert(vm.Engine.SourcePid == 0, "Manual disconnect was undone by recovery");
                vm.StopRouteCommand.Execute(null); await vm.Recover(); Assert(!vm.Engine.Running, "Manual stop was undone by recovery"); if (!replacement.HasExited) { replacement.Kill(); await replacement.WaitForExitAsync(); }
                completed.SetResult(null);
            }
            catch (Exception ex) { completed.SetResult(ex); }
