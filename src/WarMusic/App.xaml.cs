...
[System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1001:Types that own disposable fields should be disposable", Justification = "WPF owns the application lifetime; the mutex is released and disposed in OnExit.")]
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (e.Args.Contains("--smoke")) { var original = Store.FindContentRoot(AppContext.BaseDirectory); Store.Initialize(Path.Combine(original, "work", "ui-smoke")); if (File.Exists(Path.Combine(original, "data", "settings.json"))) File.Copy(Path.Combine(original, "data", "settings.json"), Path.Combine(Store.Data, "settings.json"), true); }
        else Store.Initialize(e.Args.Contains("--portable"));
        if (!e.Args.Contains("--smoke") && !OperatingSystem.IsWindowsVersionAtLeast(10, 0, 22000)) { MessageBox.Show("WarMusic requires Windows 11 or newer.", "WarMusic — unsupported Windows version", MessageBoxButton.OK, MessageBoxImage.Error); Shutdown(2); return; }
        DispatcherUnhandledException += (_, a) => { Store.Log(a.Exception.ToString()); a.Handled = true; if (!e.Args.Contains("--smoke")) MessageBox.Show("WarMusic encountered an unexpected error and must close. Details were written to diagnostics.log.\n\n" + a.Exception.Message, "WarMusic — unexpected error", MessageBoxButton.OK, MessageBoxImage.Error); Shutdown(1); };
        base.OnStartup(e);
