...
public sealed partial class MainViewModel
{
 ...
 public async Task Ready()
 {
  ...
  catch(Exception ex){Notice=ex.Message;SourceState="Waiting for saved devices · automatic recovery will retry";}
 ...
 internal async Task Recover()
 {
  ...
  try{
   ...
   if(!Engine.Routing){Engine.Start(Profile,true);Notice="Devices recovered. Music is muted; enable it when ready.";}
   ...
   if(Engine.SourcePid!=source.Pid){Engine.Panic();await Engine.ConnectSource(source.Pid);SourceState="Application reconnected · music muted";Notice="Source recovered. Enable music when ready.";}
  }catch(Exception ex){SourceState="Recovery pending · "+ex.Message;}
  finally{recovering=false;}
 }
 ...
 async Task Measure()
 {
  ...
  }catch(Exception ex){LevelStatus=ex.Message;}
 ...
 async Task Export()
 {
  ...
  Busy=true;try{Save();var snapshot=System.Text.Json.JsonSerializer.Deserialize<Settings>(System.Text.Json.JsonSerializer.Serialize(settings))!;await Task.Run(()=>PortableBackup.Export(dialog.FileName,snapshot));Notice="Backup saved with all sounds and loadouts.";}catch(Exception ex){Notice="Backup failed: "+ex.Message;}finally{Busy=false;}
 }
 async Task Restore()
 {
  ...
  }catch(Exception ex){Notice="Restore failed: "+ex.Message;}finally{Busy=false;}
 }
}
