...
{
    public bool CanExecute(object? parameter) => true; public void Execute(object? parameter) => action(parameter); public event EventHandler? CanExecuteChanged { add { } remove { } }
}
public sealed partial class MainViewModel : Observable, IDisposable
{
    ...
    public string AudioHealth { get { var h = Engine.Health; return $"Underruns {h.MonitorUnderruns + h.InputUnderruns} · overruns {h.MonitorOverruns + h.InputOverruns} · drift fixes {h.DriftCorrections} · max buffer {Math.Max(h.MaximumMonitorBufferMilliseconds, h.MaximumInputBufferMilliseconds):0.0} ms · max render {h.MaximumRenderMilliseconds:0.0} ms · reconnects {h.Reconnects}"; } }
    ...
    public ICommand EndRecordCommand { get; }
    public ICommand PreviewTestCommand { get; }
    public ICommand DeleteTestCommand { get; }
    public ICommand HelpCommand { get; }
    public ICommand CableCommand { get; }
    public ICommand WindowsCommand { get; }
    ...
    ActionCommand Cmd(Action action) => new ActionCommand(_ => Safe(action));
    void Safe(Action action) { try { action(); } catch (Exception ex) when (IsExpectedOperationFailure(ex)) { Notice = ex.Message; Store.Log(ex.ToString()); } }
    static bool IsExpectedOperationFailure(Exception exception) => exception is
        ArgumentException or
        FormatException or
        IOException or
        InvalidOperationException or
        NAudio.MmException or
    ...
    static void EscalateUnexpected(Exception exception)
    {
        Store.Log(exception.ToString());
        Application.Current.Dispatcher.BeginInvoke(
            () => throw new InvalidOperationException("Unexpected background failure.", exception));
    }
    ...
    void Refresh()
    {
        Safe(() =>
        {
            Microphones.Clear(); foreach (var d in Devices.List(DataFlow.Capture)) Microphones.Add(d); Outputs.Clear(); foreach (var d in Devices.List(DataFlow.Render)) Outputs.Add(d);
            ...
            if (!Devices.HasVirtualCable(Outputs)) Notice = "No virtual cable playback device was detected. Install or enable VB-CABLE before starting a game route.";
        });
    }
    async Task Connect()
    {
        ...
        catch (Exception ex) when (IsExpectedOperationFailure(ex)) { Notice = ex.Message; SourceState = "Unavailable · reconnect to retry"; Store.Log(ex.ToString()); }
    ...
    void RegisterHotkeys()
    {
        ...
        void Add(string key, Action action) { try { hotkeys.Register(key, () => Safe(action)); } catch (Exception ex) when (IsExpectedOperationFailure(ex)) { errors.Add(ex.Message); } }
