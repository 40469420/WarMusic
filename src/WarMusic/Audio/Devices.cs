...
public static class Devices
{
 ...
 public static AppSource ResolveApplication(AppSource selected)
 {
  ...
  foreach(var d in e.EnumerateAudioEndPoints(DataFlow.Render,DeviceState.Active))using(d)
  {
   try{var sessions=d.AudioSessionManager.Sessions;for(int i=0;i<sessions.Count;i++)using(var session=sessions[i])
   {
    try{using var process=Process.GetProcessById((int)session.GetProcessID);
