...
public sealed class ProcessCapture : IDisposable
{
    ...
    public sealed class Completion : IActivateAudioInterfaceCompletionHandler, IAgileObject
    {
        ...
        public void ActivateCompleted(IActivateAudioInterfaceAsyncOperation activateOperation) { try { activateOperation.GetActivateResult(out int hr, out object value); Marshal.ThrowExceptionForHR(hr); Result.TrySetResult(value); } catch (Exception ex) { Result.TrySetException(ex); } }
    }
}
